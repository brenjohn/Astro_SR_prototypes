...
print('Scaling data...')
scale_factor = 4
upsampler = torch.nn.Upsample(scale_factor = scale_factor)

patches_down = rescale(patches_prpd, 1/scale_factor)
patches_rstd = [upsampler(p.unsqueeze(0)).squeeze(0) for p in patches_down]

Xs_down = rescale(Xs_prpd, 1/scale_factor)
Xs_rstd = [upsampler(X.unsqueeze(0)).squeeze(0) for X in Xs_down]
...
torch.save(train_data, data_dir + 'dsc_ms_train_dataset.pth')
...
torch.save(valid_data, data_dir + 'dsc_ms_validation_dataset.pth')
...
torch.save(test_data, data_dir + 'dsc_ms_test_dataset.pth')